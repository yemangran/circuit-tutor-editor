...
function getNextComponentIndex(
  ...
  const usedIndexes = new Set(
    components
      .filter(
        (component) =>
          componentTemplates[component.kind].labelPrefix === labelPrefix,
      )
...
type CircuitStoreState = {
  ...
  upsertBranchCurrentAnnotation: (
    wireId: string,
    patch?: Partial<
      Omit<BranchCurrentAnnotation, "id" | "type" | "targetWireIds">
    >,
  ) => void;
    ...
    (set) => ({
      ...
      addComponent: (kind, position) =>
        set((state) => {
          let resolvedPosition =
            position ??
            getDefaultComponentPosition(state.doc.components.length);
      ...
      deleteComponent: (componentId) =>
        set((state) => {
          ...
          return {
            doc: {
              ...state.doc,
              components: state.doc.components.filter(
                (component) => component.id !== componentId,
              ),
              wires: state.doc.wires.filter(
                (wire) => !removedWireIds.has(wire.id),
              ),
              ...
              annotations: state.doc.annotations.filter(
                (annotation) =>
                  !(
                    ...
                    annotation.targetWireIds.some((wireId) =>
                      removedWireIds.has(wireId),
                    )
                  ) &&
      ...
      duplicateComponent: (componentId, position) =>
        set((state) => {
          ...
          const nextLabel = getNextComponentLabel(
              ...
              components: [
                ...state.doc.components,
                {
                  ...
                  parameters: Object.fromEntries(
                    Object.entries(sourceComponent.parameters).map(
                      ([key, value]) => [key, { ...value }],
                    ),
                  ),
      ...
      addComponentPin: (componentId) =>
        set((state) => {
          const component = state.doc.components.find(
            ...
            doc: {
              ...
              components: state.doc.components.map((item) =>
                item.id === componentId
                  ? {
                      ...item,
                      pins: item.pins.filter(
                        (existingPinId) => existingPinId !== pinId,
                      ),
                    }
                  : item,
              ),
      ...
      setNamedNode: (pinKey, label) =>
        set((state) => {
          ...
          const existingNamedNode = state.doc.namedNodes.find(
            (node) => node.id === pinKey,
          );
          ...
          return {
            doc: {
              ...state.doc,
              namedNodes: [
                ...state.doc.namedNodes,
                { id: pinKey, label, position },
              ],
            },
          };
        }),
      ...
      upsertControlRelation: (targetComponentId, patch) =>
        set((state) => {
          ...
          const defaultMode: ControlRelation["mode"] = isVoltageControlledSource
            ? "VCVS"
            : "CCCS";
          const defaultControlType = isVoltageControlledSource
            ? "voltage"
            : "current";
          ...
          const nextRelation: ControlRelation = {
            ...
            gain: existing?.gain ??
              targetComponent?.parameters.gain ?? { magnitude: null },
            ...patch,
          };
            ...
            doc: {
              ...
              controlRelations: existing
                ? state.doc.controlRelations.map((relation) =>
                    relation.targetComponentId === targetComponentId
                      ? nextRelation
                      : relation,
                  )
